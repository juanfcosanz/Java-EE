...
public class PersonaDaoImpl implements PersonaDao {
	...
	public Persona getPersonaByEmail(Persona persona) {
		//en vez de pasar un email, estamos pasando un obejto completo, debido a que la persona 
		//tiene como propiedad el email, y utilizando la class BeanPropertySqlParameterSource, 
		//si este objeto (Persona) contiene un atributo que tenga el mismo nombre que el parametro, 
		//se toma ese valor para complementar el query
		String sql = "SELECT * FROM PERSONA WHERE email = :email";
		SqlParameterSource nameParameters = new BeanPropertySqlParameterSource(persona);
		//si no se tiene el obejto RowMapper, se puede utilizar la siguiente linea para crear este objeto
		//RowMapper<Persona> personaRowMapper = ParameterizedBeanPropertyRowMapper.newInstance(Persona.class);
		return namedParameterJdbcTemplate.queryForObject(sql, nameParameters, new PersonaRowMapper());
	}
